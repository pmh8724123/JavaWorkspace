...
public class A_Array {
	/*
	 * 변수 : 하나의 공간에 하나의 값을 저장할 수 있는 저장공간. int a = 10; a = 20; <<-- X 배열 : 하나의 공간에
	 * 여러개의 값을 담을 수 있다.("같은 자료형"기준) ex) int[] a = new int[3]; int 자료형 값을 담을 수 있는
	 * 저장공간이 3칸 할당된다. (4byte * 3) [0, 0, 0 ] 배열에서 저장된 데이터를 꺼낼 때는 "인덱스"를 활용하며, 인덱스값은
	 * 0부터 시작. arr[0], arr[1], arr[2] 배열의 크기가 만약 5라면, 배열의 마지막 인덱스는 4 배열의 크기가 n이라면,
	 * 해달 배열의 마지막 인덱스는 항상 n-1
	 */
	...
	public void method1() {
		...
		/*
		 * 배열 할당 - 이 배열에 몇 개의 값들을 보관할건지 그 배열의 크기를 지정해주는 과정 - 크기를 지정해주면, 지정한 갯수만큼의 저장공간을
		 * 가진 배열이 생성 [표현법] 배열명 = new 자료형[배열크기];
		 * 
		 */
	...
	public void method2() {
		...
		System.out.println("iArr의 HashCoode: " + iArr.hashCode());
	...
	public void method7() {
		...
		// Arrays.toString(배열) => 배열내부의 값들을 문자열로 변환하여 반환하는 매서드.
