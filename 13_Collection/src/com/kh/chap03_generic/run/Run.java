...
public class Run {
	public static void main(String[] args) {
		 ...
		 *  - 컬렉션에 데이터를 추가 시 항상 Object객체로 저장해야 했다.
		 *  - 저장된 데이터를 꺼내서 활용시에는 항상 "강제형변환"이 필요했다.
		 *  - 단, 형변환 시 잘못된 자료형으로의 형변환을 진행한다면 에러가 발생할 수 있었다.
		 * */
		List l = new ArrayList();
		for (Object o : l) {
			// o가 String이 아닌경우 CalssCastException발생
			String s = (String) o;
		}
	 ...
	 *  - 클래싕 메서드의 "반환형", "매개변수", 필드의 "자료형"들을 미리 확정
	 ...
	 * */
		...
		// ?로 타입변수 사용 시 올바른 값을 사용중인지 검사가 불가능하기 때문에 에러가 발생..
...
//		unknown2.setType(new Child1()); // ?의 범위에 Child2도 포함되어 있으므로 
		// 대입불가능한 상황이 생긴다.
...
//		unknown2.setType(new Child2());
		...
		unknown3.setType(new Child2()); // 다형성의 업캐스팅으로 문제 없이 저장 가능.
		// 값을 "대입"하고자 할 때는 super를 통한 하한제한을 사용하는 것이 적절하다.
		
		Parent p3 = unknown3.getType(); // ?는 Object일 수 있음. 단, 클래스레벨 제네릭 설정 시
