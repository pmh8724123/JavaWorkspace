...
//	 * */
	...
	// - 문자열 데이터가 담기는 영역으로, Heap메모리영역상에 항상 고정적으로 할당되어 있다.
	public void method1() {
...
//		/* 
//		 * 자바에서는 문자열 리터럴 사용 시 상수풀에 값을 저장한다.
//		 * 
//		 * 문자열 상수풀 동작방식(jvm)
...
//		 * */
		...
		/*
		 * hashCode메서드를 오버라이딩한 이유 - 문자열은 문자열기반 hashCode값을 상수풀상의 "주소값"으로 사용하고 있다. 따라서
		 * 상수풀안에 데이터를 저장할 때 문자열을 해시코드값으로 변환한 후 그자리에 데이터가 있는지 없는지 검사를 실행하고, 이미 값이 있다면 해당
		 * 주소값을 변수에 할당 하거나 데이터가 없었다면 상수풀에 새롭게 값을 등록하고, 그 주소값을 반환한다.
		 */
