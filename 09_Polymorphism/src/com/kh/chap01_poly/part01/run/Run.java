...
//		 * */
		...
		// 3. 부모타입 참조변수로 자식타입 객체를 다루기
		System.out.println("3. 부모타입 참조변수로 자식타입 객체 다루기");
		Parent p2 = /* (Parent) */new Child1(); // 다형성이 적용된 개념

		p2.printParent(); // p2(부모타입변수)로는 Parent클래스 내부의 메서드/필드만 접근가능.
		// 자식클래스의 메서드에 접근하고자 한다면, DownCasting이 필요.
		((Child1) p2).printChild1(); // downCasting

		/*
		 * 대입연산자 기준 양쪽의 자료형이 다름에도 불구하구 오류가 나지 않는다. 이유는, 자식객체가 부모자료형으로 '자동형변환'되고 있기 때문.
		 * 상속구조의 경우 클래스간의 형변환이 가능.
		 * 
		 * 클래스간 형변환 1. UpCasting - 자식타입에서 부모타입으로 자동 형변환 - 자동형변환이 가능하므로 형병환연산자는 생략이 가능 -
		 * ex) 자식객체.부모메서드(); 부모참조변수 = 자식객체;
		 * 
		 * 2. DownCasting - 부모타입에서 자식타입으로 형변환 되는 경우 - 생략이 불가능(강제 형변환 필요) - ex) ( (자식) 부모
		 * ).자식메서드();
		 * 
		...
		arr1[1] = new Child1(1, 2, 5);

		Child2[] arr2 = new Child2[2];
		arr2[0] = new Child2(1, 2);
		arr2[1] = new Child2(3, 4);

		// 다형성ㅇ이 적용된다면, 부모타입 레퍼런스로 다양한 자식객체들을 받아줄 수 있다.
		...
		for(int i =0; i< arr.length; i++) {
			// 각 인덱스 별로 실제 참고하고 있는 자식클래스로 적절히 다운캐스팅 후, 메서드를 호출하려면?
			// 1. instanceof 자료형 연산자 활용.
			//  - 객체가 실제로 참조하고있는 클래스를 확인하여 자료형과 같다면 true / 다르면 false 반환
